  ...
  return <div className="min-h-screen bg-[#f7f7f6] text-black">
    ...
    <main>
      <header className="border-b border-black/10 bg-white py-14 md:py-20"><div className="page-shell grid gap-8 md:grid-cols-[220px_minmax(0,1fr)] md:items-center">
        ...
        <div><a href="/authors" className="text-[10px] font-semibold uppercase tracking-[0.14em] text-black/40">← All authors</a><h1 className="mt-5 font-display text-5xl leading-none md:text-7xl">{author.name}</h1><p className="mt-4 text-xs font-medium text-black/45">{[author.role, author.location, author.country].filter(Boolean).join(' · ')}</p><p className="mt-7 max-w-3xl text-base leading-8 text-black/60">{author.bio || 'This author has not yet added a biography.'}</p></div>
      </div></header>
