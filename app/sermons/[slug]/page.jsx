...
export default async function SermonPage({ params }) {
  ...
  return <><Navbar /><main className="bg-parchment-ivory pb-20 pt-32 md:pt-40"><article>
    ...
    <section className="page-shell mt-10"><div className="overflow-hidden bg-midnight-navy shadow-xl">{sermon.videoUrl ? (embedUrl ? <iframe src={embedUrl} title={sermon.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="aspect-video w-full" /> : <video src={sermon.videoUrl} poster={sermon.image || undefined} controls className="aspect-video w-full bg-black object-contain" />) : <div className="relative grid min-h-[360px] place-items-center overflow-hidden p-8 text-center">{sermon.image && <img src={sermon.image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" />}<div className="absolute inset-0 bg-gradient-to-t from-midnight-navy via-midnight-navy/45 to-transparent" /><div className="relative"><span className="material-symbols-outlined text-6xl text-white">headphones</span><p className="mt-4 text-[10px] font-bold uppercase tracking-[0.18em] text-white/65">Audio sermon</p><h2 className="mt-2 font-display text-3xl text-white">Listen to {sermon.title}</h2></div></div>}</div>
