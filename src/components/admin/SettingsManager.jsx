        ...
        <div className="space-y-6">
          ...
          <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm md:p-7">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2">
              <label className="text-xs font-semibold text-slate-500">Site name<input required value={settings.siteName} onChange={update('siteName')} className={fieldClass} /></label>
              <label className="text-xs font-semibold text-slate-500">Editorial contact email<input required type="email" value={settings.contactEmail} onChange={update('contactEmail')} className={fieldClass} /></label>
              <label className="text-xs font-semibold text-slate-500 md:col-span-2">Site description<textarea value={settings.siteDescription} onChange={update('siteDescription')} rows={3} className={`${fieldClass} resize-y`} /></label>
              <div className="text-xs font-semibold text-slate-500"><p>Timezone</p><AdminSelect variant="field" label="Timezone" value={settings.timezone} onChange={(value) => setSettings((current) => ({ ...current, timezone: value }))} options={['Africa/Lagos', 'Africa/Accra', 'Africa/Johannesburg', 'Africa/Nairobi', 'Asia/Dubai']} /></div>
            </div>
          </section>
