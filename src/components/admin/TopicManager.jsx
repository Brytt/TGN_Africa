...
export default function TopicManager({ initialTopics = [] }) {
  ...
  const updateTopicInTree = (id, title) => {
    setTopics((current) => current.map((topic) => {
      ...
      return {
        ...
        subtopics: topic.subtopics.map((sub) => {
          ...
          return { ...sub, resources: sub.resources.map((item) => item.id === id ? { ...item, title } : item) }
        }),
      }
    }))
  }

  const removeTopicFromTree = (id) => {
    setTopics((current) => current
      ...
      .map((topic) => ({
        ...
        subtopics: topic.subtopics
          ...
          .map((sub) => ({ ...sub, resources: sub.resources.filter((item) => item.id !== id) })),
      })))
  }

  const startEditing = (item, label) => {
    setEditing({ id: item.id, label })
  ...
  const saveEdit = async (event) => {
    ...
    const response = await fetch(`/api/admin/topics/${editing.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    setBusyId('')
    if (!response.ok) {
      setNotice(result.error || 'Unable to update this topic.')
      return
    }
    updateTopicInTree(editing.id, result.data.title)
    setEditing(null)
    setEditName('')
    setNotice(`${result.data.title} updated. Existing links remain unchanged.`)
  }

  const deleteTopic = async (item, label) => {
    const warning = label === 'main topic'
      ? `Delete “${item.title}” and every subtopic beneath it? Publications will remain available but become uncategorized.`
      : label === 'subtopic'
        ? `Delete “${item.title}” and its subsections? Publications will remain available but become uncategorized.`
        : `Delete “${item.title}”? Publications will remain available but become uncategorized.`
    if (!window.confirm(warning)) return

    setBusyId(item.id)
    const response = await fetch(`/api/admin/topics/${item.id}`, { method: 'DELETE' })
    const result = await response.json()
    setBusyId('')
    if (!response.ok) {
      setNotice(result.error || 'Unable to delete this topic.')
      return
    }
    removeTopicFromTree(item.id)
    setNotice(`${item.title} deleted. No publications were deleted.`)
  }

  const TopicActions = ({ item, label }) => (
    <span className="flex shrink-0 items-center gap-1">
      <button type="button" onClick={() => startEditing(item, label)} disabled={busyId === item.id} className="grid h-8 w-8 place-items-center rounded-full text-slate-400 hover:bg-white hover:text-midnight-navy disabled:opacity-40" aria-label={`Edit ${item.title}`} title="Edit"><span className="material-symbols-outlined text-[17px]">edit</span></button>
      <button type="button" onClick={() => deleteTopic(item, label)} disabled={busyId === item.id} className="grid h-8 w-8 place-items-center rounded-full text-slate-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-40" aria-label={`Delete ${item.title}`} title="Delete"><span className="material-symbols-outlined text-[17px]">delete</span></button>
    </span>
  )
  ...
  return (
    <main className="admin-scroll min-h-0 flex-1 overflow-y-auto px-6 pb-10 pt-6 xl:px-10">
      <div className="mx-auto max-w-[1180px]">
        ...
        {editing && (
          <form onSubmit={saveEdit} className="mb-6 rounded-3xl border border-heritage-gold/25 bg-white p-5 shadow-sm md:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-end">
              <label className="min-w-0 flex-1 text-xs font-semibold text-slate-500">
                Edit {editing.label}
                <input required autoFocus value={editName} onChange={(event) => setEditName(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-normal text-slate-900 outline-none focus:border-midnight-navy/30 focus:ring-2 focus:ring-midnight-navy/10" />
              </label>
              <div className="flex gap-2">
                <button type="button" onClick={() => setEditing(null)} className="rounded-full border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
                <button type="submit" disabled={busyId === editing.id || !editName.trim()} className="rounded-full bg-midnight-navy px-5 py-2.5 text-sm font-medium text-white disabled:opacity-40">{busyId === editing.id ? 'Saving…' : 'Save changes'}</button>
              </div>
            </div>
            <p className="mt-3 text-xs text-slate-400">The existing page URL will be preserved.</p>
          </form>
        )}
            ...
            {matchingTopics.map((topic) => {
              ...
              return (
                <article key={topic.slug} className="overflow-hidden rounded-2xl border border-slate-100">
                  <div className="flex items-center pr-3 hover:bg-slate-50">
                  <button type="button" onClick={() => setExpanded((current) => current.includes(topic.title) ? current.filter((item) => item !== topic.title) : [...current, topic.title])} className="flex min-w-0 flex-1 items-center gap-3 px-4 py-4 text-left">
                  ...
                  <TopicActions item={topic} label="main topic" />
                  </div>
                  {topicOpen && (
                      ...
                      {topic.subtopics.map((sub) => {
                        ...
                        return (
                          <div key={sub.slug} className="border-b border-slate-100 last:border-0">
                            ...
                            <button type="button" onClick={() => setExpandedSubs((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} className="flex min-w-0 flex-1 items-center gap-3 py-3 text-left">
                            ...
                            <TopicActions item={sub} label="subtopic" />
                            </div>
                            {subOpen && <ul className="mb-3 ml-8 grid gap-1 border-l border-slate-200 pl-4 md:grid-cols-2">
                              {sub.resources.map((item) => <li key={item.id} className="flex items-center gap-2 rounded-lg px-3 py-1 text-xs leading-5 text-slate-500 hover:bg-white"><span className="min-w-0 flex-1 py-1">{item.title}</span><TopicActions item={item} label="subsection" /></li>)}
                            </ul>}
                          </div>
                        )
                      })}
