...
export default function TopicPage({ topics = [], topic = null, publications = [] }) {
  ...
  const visibleSubtopics = useMemo(() => {
    ...
    return topic.subtopics.map((subtopic) => ({
      ...subtopic,
      resources: subtopic.resources.filter((resource) => resource.title.toLowerCase().includes(normalized)),
    })).filter((subtopic) => subtopic.title.toLowerCase().includes(normalized) || subtopic.resources.length)
  }, [query, topic])
  ...
  return (
    <div className="min-h-screen bg-white text-charcoal-text">
      ...
      {!topic ? (
        ...
        <main>
          <section className="border-b border-midnight-navy/10 bg-surface-container-low py-12 md:py-20"><div className="page-shell"><a href="/topics" className="text-[10px] font-bold uppercase tracking-[0.16em] text-midnight-navy/50">← All topics</a><h1 className="mt-8 max-w-4xl font-display text-4xl text-midnight-navy md:text-6xl">{topic.title}</h1><p className="mt-5 text-sm text-midnight-navy/55">{topic.subtopics.length} subtopics · {topic.subtopics.reduce((sum, item) => sum + item.resources.length, 0)} subsections</p></div></section>
          <section className="py-12 md:py-16"><div className="page-shell grid gap-10 lg:grid-cols-[260px_minmax(0,1fr)]"><aside><label className="relative block"><span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-lg text-midnight-navy/35">search</span><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full border border-midnight-navy/15 py-3 pl-10 pr-3 text-sm outline-none" placeholder="Search this topic" /></label></aside><div className="space-y-10">{visibleSubtopics.map((subtopic) => <article key={subtopic.id}><h2 className="font-display text-3xl text-midnight-navy">{subtopic.title}</h2><div className="mt-5 grid gap-2 md:grid-cols-2">{subtopic.resources.map((resource) => <div key={resource.id} className="border border-midnight-navy/10 p-4 text-sm text-midnight-navy/65">{resource.title}</div>)}</div></article>)}</div></div></section>
          {publications.length > 0 && <section className="bg-surface-container-low py-14"><div className="page-shell"><h2 className="font-display text-3xl text-midnight-navy">Published in this topic</h2><div className="mt-6 grid gap-4 md:grid-cols-3">{publications.map((item) => <a key={item.id} href={`/articles/${item.slug}`} className="bg-white p-5"><p className="text-xs font-semibold text-midnight-navy">{item.title}</p><p className="mt-2 text-[10px] text-midnight-navy/45">{item.author} · {item.type}</p></a>)}</div></div></section>}
        </main>
      )}
      <Footer />
    </div>
